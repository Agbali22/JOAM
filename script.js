...
const homeCardRow = document.querySelector('.home-grid');
const scrollCue = document.querySelector('.scroll-cue');

function getHomeScrollDestination() {
  if (!homeContent) return 0;

  const contentTop = homeContent.getBoundingClientRect().top + window.scrollY;

  if (!homeCardRow) return contentTop;

  const cardRect = homeCardRow.getBoundingClientRect();
  const cardBottom = cardRect.bottom + window.scrollY;
  const desktopCardsFit = window.innerWidth > 840 && cardRect.height <= window.innerHeight;

  if (!desktopCardsFit) return contentTop;

  return Math.max(contentTop, cardBottom - window.innerHeight);
}

if (scrollCue && homeContent) {
  scrollCue.addEventListener('click', () => {
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    window.scrollTo({
      top: Math.round(getHomeScrollDestination()),
