...
const recentFrames = document.querySelector('#recent-frames');
const scrollCue = document.querySelector('.scroll-cue');

function getRecentFramesDestination() {
  if (!recentFrames) return 0;

  const header = document.querySelector('.site-header');
  const sectionTop = recentFrames.getBoundingClientRect().top + window.scrollY;
  const headerHeight = header?.getBoundingClientRect().height ?? 0;

  return Math.max(0, sectionTop - headerHeight);
}

if (scrollCue && recentFrames) {
  scrollCue.addEventListener('click', () => {
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    window.scrollTo({
      top: Math.round(getRecentFramesDestination()),
      behavior: reduceMotion ? 'auto' : 'smooth'
    });
  });
}

// Add verified observations here when real tracking data is available.
// Example shape only: { label: 'Course or exam cycle', hours: 0, result: 0 }
const studyOutcomeData = [];

const studyChart = document.querySelector('[data-study-chart]');

if (studyChart) {
  const svgNamespace = 'http://www.w3.org/2000/svg';
  const pointLayer = studyChart.querySelector('[data-study-points]');
  const trendLayer = studyChart.querySelector('[data-study-trend]');
  const emptyMessage = studyChart.querySelector('[data-study-empty]');
  const status = studyChart.querySelector('[data-study-chart-status]');
  const description = studyChart.querySelector('#study-chart-description');
  const observations = studyOutcomeData.filter(({ label, hours, result }) => (
    typeof label === 'string' &&
  ...
  if (pointLayer && trendLayer && emptyMessage && observations.length > 0) {
    emptyMessage.remove();
    ...
    const top = 42;
    const bottom = 228;
    const maxHours = Math.max(1, Math.ceil(Math.max(...observations.map(({ hours }) => hours))));
    const xPosition = (hours) => left + (hours / maxHours) * (right - left);
    const yPosition = (result) => bottom - (result / 100) * (bottom - top);

    observations.forEach(({ label, hours, result }) => {
      ...
      point.setAttribute('cx', String(xPosition(hours)));
      point.setAttribute('cy', String(yPosition(result)));
      point.setAttribute('r', '7');
      point.setAttribute('tabindex', '0');
      point.setAttribute('role', 'img');
      point.setAttribute('aria-label', `${label}: ${hours} focused hours per week, ${result}% result`);
      tooltip.textContent = `${label} â€” ${hours} focused hrs/week â€” ${result}%`;
      point.append(tooltip);
      pointLayer.append(point);
    });

    const uniqueHours = new Set(observations.map(({ hours }) => hours));

    if (observations.length >= 2 && uniqueHours.size >= 2) {
      const count = observations.length;
      const sumX = observations.reduce((sum, item) => sum + item.hours, 0);
      const sumY = observations.reduce((sum, item) => sum + item.result, 0);
      const sumXY = observations.reduce((sum, item) => sum + item.hours * item.result, 0);
      const sumXX = observations.reduce((sum, item) => sum + item.hours * item.hours, 0);
      const slope = (count * sumXY - sumX * sumY) / (count * sumXX - sumX * sumX);
      const intercept = (sumY - slope * sumX) / count;
      const line = document.createElementNS(svgNamespace, 'path');
      const startResult = Math.max(0, Math.min(100, intercept));
      const endResult = Math.max(0, Math.min(100, intercept + slope * maxHours));
      line.setAttribute('d', `M${left} ${yPosition(startResult)} L${right} ${yPosition(endResult)}`);
      trendLayer.append(line);
    }

    if (status) status.textContent = `${observations.length} observation${observations.length === 1 ? '' : 's'} tracked`;
    if (description) description.textContent = `${observations.length} verified study and course-result observation${observations.length === 1 ? '' : 's'}.`;
  }
}
  ...
  const photoLocations = [
    {
      ...
      image: './images/museum-women-movement-portrait.jpg',
    ...
    },
    {
      ...
      image: './images/boarding-pass-nikon-travel.jpg',
    ...
    },
    {
      ...
      image: './images/hair-blowout-portrait-01.jpg',
    ...
    },
    {
      ...
      image: './images/dc-metro-smartrip-card.jpg',
    ...
    },
    {
      ...
      image: './images/metro-station-overlook.jpg',
    ...
    },
    {
      ...
      image: './images/featured-photo-1.jpg',
    ...
    },
    {
      ...
      place: 'National Museum of African American History and Culture, Washington, DC',
      image: './images/washington-monument-framed.jpg',
    ...
    },
    {
      ...
      place: 'Foggy Bottom-GWU Metro Station, Washington, DC',
      image: './images/underground-metro-platform.jpg',
    ...
    },
    {
      ...
      image: './images/freer-gallery-exterior.jpg',
    ...
    },
    {
      ...
      place: 'Old Bainbridge Road, Tallahassee, Florida',
      image: './images/hair-blowout-portrait-02.jpg',
    ...
    },
    {
      ...
      image: './images/yellow-blue-vintage-airplane.jpg',
    ...
    },
    {
      ...
      image: './images/featured-photo-4.jpg',
    ...
    },
    {
      ...
      image: './images/smithsonian-garden-red-buildings.jpg',
    ...
    },
    {
      ...
      place: 'Smithsonian National Air and Space Museum, Washington, DC',
      image: './images/american-airlines-vintage-plane.jpg',
    ...
    },
    {
      ...
      image: './images/walking-dc-sidewalk-back-view.jpg',
    ...
    },
    {
      ...
      image: './images/smithsonian-castle-towers.jpg',
    ...
    },
    {
      ...
      image: './images/hotel-lobby-tablet-work.jpg',
    ...
    },
    {
      ...
      image: './images/washington-dc-avenue.jpg',
    ...
    },
    {
      ...
      image: './images/alley-camera-self-portrait.jpg',
